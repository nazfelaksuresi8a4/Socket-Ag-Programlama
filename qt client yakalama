#Bu kod basit ve prototip bir İstek yakalama sistemidir 
#Kodda mesaj yakalama,bağlanan cihazın ip adresini görme bağlanan cihazın portunu görme gibi seçenekler bulunmaktadır bu bir proje değildir.
...
class Thread(QObject):
    ...
    def streamLoop(self):
        while self.stream_event:
            ...
            self.signal.emit(str(message),str(self.client_ip),str(self.client_port))
...
class mainWindow(QMainWindow):
    ...
    def Emitter(self):
        ...
        self.signalClass.signal.connect(self.updatef)

        self.index = 0
        self.connections_index = 0
        self.connections_list = []
    
    def updatef(self,msg,ip,port):
        if len(self.connections_list) == 2:
            conn = len(set(self.connections_list))
            self.connections.setText(f'Baglantilar: {conn}')

        self.index += 1

        self.connections_list.append(str(ip))

        QMessageBox.warning(self,f'BAĞLANTI GERÇEKLEŞTİ',f'BAGLANAN İP: {ip}\nBAGLANAN PORT: {port}\nMESAJ: {msg}')

        self.signal_1_btn.setText(str(self.index))
